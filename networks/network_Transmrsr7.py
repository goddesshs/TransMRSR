...
# class FAB
#     R"""
#encoder-decoder块后再加入深层特征提取
#加入SFT
#         Feature extraction:
#******没有hr 
#     """
class TGP7(nn.Module):
    r""" 
    ...
    def __init__(self,
        ...
        patch_norm=True,
        ape = False, 
        img_range=1., sft_half = False, fix_decoder=False, pretrained_path=None, truncation_path=None):
        print('ffffffff', fix_decoder)
        
        super(TGP7, self).__init__()
        num_in_ch = in_chans
        start = 2
    ...
    def forward(self, tar):
        ...
        # tar_lr = tar_lr.view(tar_lr.shape[0], out_size[0]*out_size[1], -1)
        # ref_hr = tar_lr.view(tar_lr.shape[0], out_size[0]*out_size[1], -1)
            # tar
        #B,4*4,C
        ...
        #B,L,C->B,C,H,W
        # tar_lr = self.patch_unembed(tar_lr, (4,4))
        # tar_lr = self.final_layers(tar_lr)
        #B, num_layers, 
